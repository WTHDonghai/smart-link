...
export const ERROR_DICTIONARY: Record<AppErrorCode, AppErrorDefinition> = {
  ...
  NET_NOT_FOUND: {
    code: 'NET_NOT_FOUND',
    domain: 'NET',
    userTitle: '服务接口或资源不存在 (404)',
    userMessage: '请求的目标服务接口或服务端资源未找到 (HTTP 404)，可能服务版本未更新或模块路由变动。',
    suggestion: '请检查服务模块配置，或联系技术运维人员确认接口版本与路由。',
    retryable: false,
  },
  NET_BAD_REQUEST: {
    code: 'NET_BAD_REQUEST',
    domain: 'NET',
    userTitle: '请求参数校验失败 (400)',
    userMessage: '提交的请求数据不符合服务端参数校验规范 (HTTP 400)。',
    suggestion: '请检查输入内容格式或必填项后重试。',
    retryable: false,
  },
  NET_FORBIDDEN: {
    code: 'NET_FORBIDDEN',
    domain: 'NET',
    userTitle: '操作权限受限 (403)',
    userMessage: '当前账号或客户端无权访问该接口或资源 (HTTP 403)。',
    suggestion: '请联系管理员开通对应模块的操作权限。',
    retryable: false,
  },
  NET_SERVER_ERROR: {
    code: 'NET_SERVER_ERROR',
    domain: 'NET',
    userTitle: '平台服务处理异常 (500)',
    userMessage: '文旅平台后端服务在处理该请求时发生内部错误 (HTTP 500)。',
    suggestion: '平台底层服务可能在繁忙或维护中，请稍后重试。',
    retryable: true,
  },
  SYS_CONFIG_MISSING: {
