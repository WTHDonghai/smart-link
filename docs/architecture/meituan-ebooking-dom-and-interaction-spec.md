...
#### 二次确认弹窗处理
点击“查看姓名”后，美团会弹出 MTD / Xigua 安全确认框：
- 确认按钮：`.mtd-modal .mtd-btn:has-text("我已知晓")` 或 `.mtd-confirm button:has-text("确定")`。
- 系统必须自动点击确认以完成解密。
...
| **解密客人真实姓名** | 裸文本匹配 `button:has-text("查看姓名")` | 基于 DOM 结构与样式定位：`.detail-info-item .guest-name .btn-text` 并自动点击平台安全弹窗 |
| **解密客人联系电话** | 无脑每次都去点击“查看电话” | 若姓名解密或原始报文已有明文手机号，**强制跳过**解密以规避双重敏感风控探针 |
